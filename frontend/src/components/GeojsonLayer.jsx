...
export default function GeojsonLayer({db_name, title, ip_address}) {
    ...
    useEffect(() => {
        ...
        fetch(`http://${ip_address}:8000/minmaxts?db_name=${db_name}`)
    ...
    useEffect(() => {
        setFetching(true)
        fetch(`http://${ip_address}:8000/geojson?limit=${limit}&db_name=${db_name}`).then((res) => res.json()).then((data) => {
